  ...
  <div class="rule-editor">
    <a-form layout="vertical" size="small">
      ...
      <a-form-item label="计算方式">
        <a-select v-model:value="cParams.template" @change="onTemplateChange">
      ...
      <!-- 生成的表达式预览（非自定义模式） -->
      <a-form-item v-if="!isCustom && cParams.expression" label="表达式预览">
      ...
      <a-form-item v-if="isCustom" label="表达式">
        <ExpressionEditor
          v-model="cParams.expression"
          :components="components"
        />
      </a-form-item>

      <a-form-item label="保留小数位">
        <a-input-number v-model:value="cParams.precision" :min="0" :max="6" style="width: 100%" />
      </a-form-item>
    </a-form>
  </div>
</template>
...
import type { CalculationParams, ComponentDef, RuleDef } from '../../types'
...
const safeDefault: RuleDef = {
  ...
  enabled: true,
  trigger: 'onChange',
  params: { template: 'sum', expression: '', precision: 2 }
}

const local = reactive<RuleDef>(
  props.rule ? JSON.parse(JSON.stringify(props.rule)) : safeDefault
)
const cParams = local.params as CalculationParams
...
const isMultiFieldTemplate = computed(() =>
  ['sum', 'average', 'max', 'min'].includes(cParams.template)
)
const isTwoFieldTemplate = computed(() =>
  ['percent', 'subtract'].includes(cParams.template)
)
const isCustom = computed(() => cParams.template === 'custom')

/** 将表达式中的 $.components.xxx.value 替换为可读的字段标签 */
const readableExpression = computed(() => {
  const expr = cParams.expression || ''
  if (!expr) return ''
...
function generateExpression() {
  const tpl = cParams.template
  if (isMultiFieldTemplate.value) {
    const refs = selectedFields.value.map(id => `$.components.${id}.value`)
    if (tpl === 'sum')
      cParams.expression = refs.join(' + ') || ''
    else if (tpl === 'average')
      cParams.expression = refs.length ? `(${refs.join(' + ')}) / ${refs.length}` : ''
    else if (tpl === 'max')
      cParams.expression = refs.length ? `Math.max(${refs.join(', ')})` : ''
    else if (tpl === 'min')
      cParams.expression = refs.length ? `Math.min(${refs.join(', ')})` : ''
    ...
    if (tpl === 'percent')
      cParams.expression = `(${a}) / (${b}) * 100`
    else if (tpl === 'subtract')
      cParams.expression = `${a} - ${b}`
  }
  // custom 模式不自动生成
}
...
function onTemplateChange(val: string) {
  const ids = parseFieldIds(cParams.expression)
