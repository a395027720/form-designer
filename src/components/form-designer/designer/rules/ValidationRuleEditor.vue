<template>
  <div class="rule-editor">
    <a-form layout="vertical" size="small">
      ...
      <div v-for="(v, i) in vParams.validators" :key="i" class="validator-row">
...
import type { RuleDef, ValidationParams, ValidationValidator } from '../../types'
...
// 校验规则专属：把 params 收敛为 ValidationParams（其他类型在 conditional 时不会进来）
const vParams = local.params as ValidationParams

const add = () => {
  if (!vParams.validators) vParams.validators = []
  vParams.validators.push({ type: 'required', message: '此项必填' } as ValidationValidator)
}

const remove = (i: number) => {
  if (!vParams.validators) return
  vParams.validators.splice(i, 1)
}
