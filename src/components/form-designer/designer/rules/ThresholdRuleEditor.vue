<template>
  <div class="rule-editor">
    <a-form layout="vertical" size="small">
      ...
      <div v-for="(r, i) in tParams.ranges" :key="i" class="range-row">
...
import type { RuleDef, ThresholdParams, ThresholdRange } from '../../types'
...
const local = reactive<RuleDef>(
  (() => {
    ...
    const r = props.rule
    if (r && (r.params as ThresholdParams)?.ranges && (r.params as ThresholdParams).ranges.length > 0) {
...
const tParams = local.params as ThresholdParams

const hasAction = (type: string) =>
  (props.rule?.params as ThresholdParams | undefined)?.actions?.some((a: any) => a.type === type) ?? false
...
const emitUpdate = () => {
  ...
  tParams.actions = actions
  emit('update:rule', JSON.parse(JSON.stringify(local)))
}

const add = () => {
  tParams.ranges.push({ min: 0, max: 0, label: '新区间', level: 'normal', color: '#1890ff' })
  emitUpdate()
}

const remove = (i: number) => {
  tParams.ranges.splice(i, 1)
  emitUpdate()
}
