 ...
 */
import type { ComponentDef, RuleDef, ThresholdParams, ComparisonParams, ConditionalParams, CalculationParams } from '../types'
...
export class RuleEngine {
  ...
  private runThreshold(owner: ComponentDef, rule: RuleDef) {
    ...
    const raw = this.values.get(owner.id)
    // 空值不触发阈值规则（避免空字符串被 Number('') 当成 0 误判为"偏低"）
  ...
  private runConditional(rule: RuleDef) {
    ...
    // 先撤销"另一侧"的副作用，再执行当前侧的操作，避免状态残留
    const [revert, apply] = matched
      ? [p.else, p.then]
      : [p.then, p.else]

    for (const action of revert) {
      if (!action.target) continue
      const revertType = action.type === 'show' ? 'hidden' : 'show'
      this.applier.apply({ type: revertType, target: action.target }, '')
      const m = action.target.match(/\$\.components\.([^.]+)/)
      if (m) this.visibility.set(m[1], revertType === 'show')
    }
    for (const action of apply) {
      if (!action.target) continue
      this.applier.apply({ type: action.type === 'show' ? 'show' : 'hidden', target: action.target }, '')
      const m = action.target.match(/\$\.components\.([^.]+)/)
      if (m) this.visibility.set(m[1], action.type === 'show')
    }
  }
  ...
  private readSide(side: string | { type: 'constant'; value: any }): any {
    ...
    if (typeof side === 'string') {
      ...
      if (m) {
        const v = this.values.get(m[1])
        // 空值（undefined/null/''）视为"未填"，规则不应触发
