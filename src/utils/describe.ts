 ...
 */
import type { RuleDef, ComponentDef, ThresholdParams, ComparisonParams, ConditionalParams, ValidationParams, CalculationParams } from '@/types/template'
...
function describeComparison(p: ComparisonParams, comps: ComponentDef[]): string {
  const left = typeof p.left === 'string' ? refLabel(p.left, comps) : constLabel(p.left)
  const right = typeof p.right === 'string' ? refLabel(p.right, comps) : constLabel(p.right)
...
function describeConditional(p: ConditionalParams, comps: ComponentDef[]): string {
  ...
  const right = typeof p.when.right === 'string' ? refLabel(p.when.right, comps) : constLabel(p.when.right)
  const thenTarget = compAction(p.then?.type || 'show', p.then?.target || '', comps)
  const elseTarget = p.else ? compAction(p.else.type, p.else.target, comps) : null
  let s = `当 ${left} ${opLabel(p.when.operator)} ${right} 时，${thenTarget}`
  if (elseTarget) s += `；否则 ${elseTarget}`
  return s
}
